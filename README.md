...
- **mootdx 连接优化**：先尝试 `bestip=True` 自动选优，失败后逐个尝试8个最新通达信服务器地址，全部失败自动回退到 AKShare
...
# 指定股票代码
python -m src.main --stocks sh600519 sz002843
...
**交易日判断**：系统自动识别交易日，周末和节假日不会重复请求已更新的数据。
...
mootdx 采集器采用多层连接策略确保成功率：

1. **自动选优**：先尝试 `bestip=True`，让 mootdx 自动测速并选择最快的可用服务器
2. **预设服务器**：自动选优失败后，逐个尝试8个最新通达信服务器地址（2025年更新）
3. **优雅降级**：全部服务器失败后，自动回退到 AKShare 获取数据

### 服务器地址列表

mootdx 采集器内置以下通达信服务器（`src/collector/mootdx_collector.py`）：

| 服务器 | IP地址 | 端口 | 来源 |
|--------|--------|------|------|
| 申银万国上海电信1 | 222.73.235.23 | 7709 | 申银万国 |
| 申银万国上海电信2 | 222.73.235.29 | 7709 | 申银万国 |
| 申银万国成都电信 | 125.64.39.61 | 7709 | 申银万国 |
| 国金上海电信 | 222.73.56.72 | 7709 | 国金证券 |
| 华西联通L1 | 119.4.167.141 | 7709 | 华西证券 |
| 上海双线资讯主站 | 121.36.199.182 | 7711 | 公开地址 |
| 上海双线资讯主站2 | 124.70.158.189 | 7711 | 公开地址 |
| 备用地址 | 119.147.212.81 | 7709 | 旧地址 |

### 常见问题

**Q: mootdx 连接失败怎么办？**

A: 系统会自动回退到 AKShare 获取数据，不影响整体采集流程。如果长期失败，可以：
1. 检查网络是否能访问 7709/7711 端口
2. 检查是否有防火墙阻止 TCP 连接
3. 更新 `DEFAULT_SERVERS` 列表中的服务器地址

**Q: 为什么日志显示 "所有服务器连接失败"？**

A: 通达信服务器地址可能会定期变更，系统会自动降级到 AKShare。可以更新 `src/collector/mootdx_collector.py` 中的服务器列表。

## Tushare 配置（北向资金回退源）

北向资金数据采集采用 **AKShare主源 + Tushare回退** 策略。

**配合缓存机制**：由于系统默认启用24小时缓存，且智能跳过非交易日，实际Tushare API调用频率大幅降低。

### 数据源状态

| 数据源 | 接口 | 状态 | 说明 |
|--------|------|------|------|
| AKShare | `stock_hsgt_individual_em` | ⚠️ 数据停更 | 接口可用，但数据仅更新至2024-08-16 |
| Tushare Pro | `hsgt_top10_detail_em` | ✅ 可用 | 需Pro版token，积分权限要求 |

### 配置方法

1. **获取 Tushare Pro Token**：访问 [Tushare官网](https://tushare.pro/) 注册并获取 token
...
当 AKShare 北向资金数据过期（最新日期距今天超过30天）时，系统自动尝试 Tushare Pro 回退：

```
北向资金采集流程:
├── 1. 尝试 AKShare (stock_hsgt_individual_em)
├── 2. 检查数据是否过期 (>30天)
│   ├── 未过期 → 结束
│   └── 已过期 → 尝试 Tushare Pro 回退
└── 3. Tushare Pro 采集 (需配置 token)
```

**注意**：Tushare Pro 需要足够积分才能访问个股北向资金接口。免费用户仅能获取市场整体数据。
